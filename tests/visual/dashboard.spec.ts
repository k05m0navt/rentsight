...
test.describe('Dashboard visual regression tests', () => {
  for (const theme of themes) {
    ...
    test(`dashboard ${theme} theme - tablet`, async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(`dashboard-${theme}-tablet.png`, {
        fullPage: true,
        maxDiffPixels: 2000,
      });
    });
    ...
    test(`dashboard ${theme} theme - mobile`, async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(`dashboard-${theme}-mobile.png`, {
        fullPage: true,
        maxDiffPixels: 2000,
      });
    });
    ...
    test(`dashboard metrics cards ${theme} theme`, async ({ page }) => {
      ...
      if ((await metricsCards.count()) > 0) {
        await expect(metricsCards).toHaveScreenshot(`dashboard-metrics-${theme}.png`, {
          maxDiffPixels: 100,
        });
      }
    });
    ...
    test(`dashboard charts ${theme} theme`, async ({ page }) => {
      ...
      const chartContainer = page
        .locator('[class*="ChartContainer"], [data-testid*="chart"]')
        .first();

      if ((await chartContainer.count()) > 0) {
        await expect(chartContainer).toHaveScreenshot(`dashboard-charts-${theme}.png`, {
