...
test.describe('Spacing Consistency Analysis', () => {
  ...
  test('should maintain consistent button spacing', async ({ page }) => {
    ...
    if (count > 1) {
      ...
      const firstPadding = await firstButton.evaluate((el) => window.getComputedStyle(el).padding);
  ...
  test('should maintain spacing in navigation items', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const padding = await firstItem.evaluate((el) => window.getComputedStyle(el).padding);
  ...
  test('should have consistent heading margins', async ({ page }) => {
    ...
    if (count > 0) {
      for (let i = 0; i < Math.min(count, 3); i++) {
        ...
        const margin = await heading.evaluate((el) => window.getComputedStyle(el).marginBottom);
  ...
  test('should have consistent list item spacing', async ({ page }) => {
    ...
    if ((await listItems.count()) > 0) {
      ...
      const marginBottom = await firstItem.evaluate(
