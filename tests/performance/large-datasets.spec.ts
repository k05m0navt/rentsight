...
test.describe('Performance tests for large datasets', () => {
    ...
    if (await table.count() > 0) {
      ...
      const scrollPerformance = await page.evaluate(() => {
        return new Promise<number>((resolve) => {
          const scrollContainer = document.querySelector('[data-testid="rent-table"], [class*="overflow"]');
          if (!scrollContainer) {
  ...
  test('chart data aggregation handles 10,000+ entries efficiently', async ({ page }) => {
    ...
    const hasCharts = await page.locator('[data-testid="analytics-chart"], svg, canvas').count() > 0;
  ...
  test('filtering large datasets responds quickly', async ({ page }) => {
    ...
    const filterButton = page.locator('[data-testid="filter-button"], button[class*="filter"]').first();
  ...
  test('pagination performance with large datasets', async ({ page }) => {
    ...
    const nextPageButton = page.locator('[aria-label*="next"], button[class*="pagination"]').first();
